...
public class Thumb2 extends ShellCode {
    ...
    public byte[] createBridgeJump(long targetAddress, long targetEntry, 
        ...
        // 有问题，参数丢失。
//        byte[] instructions = new byte[] {
            // // ldr ip, [pc, #24], ip = source_method_address
            // (byte) 0xdf, (byte) 0xf8, 0x18, (byte) 0xc0,    
...
//                instructions.length - 4);
        ...
        byte[] instructions = new byte[]{
            ...
            (byte) 0x40, (byte) 0xf0, (byte) 0x19, (byte) 0x80, 
            // ldr r0, [pc, #28] r0 = target_method_address
            (byte) 0x08, (byte) 0x48,                           
            // ldr ip, [pc, #38] ip = struct address
            (byte) 0xdf, (byte) 0xf8, (byte) 0x28, (byte) 0xc0, 
            // str sp, [ip, #0]
            (byte) 0xcc, (byte) 0xf8, (byte) 0x00, (byte) 0xd0, 
            // str r2, [ip, #4]
            (byte) 0xcc, (byte) 0xf8, (byte) 0x04, (byte) 0x20, 
            // str r3, [ip, #8]
            (byte) 0xcc, (byte) 0xf8, (byte) 0x08, (byte) 0x30, 

            // mov r3, ip
            (byte) 0x63, (byte) 0x46,                           
            // ldr r2, [pc, #16] r2 = source_method_address
            (byte) 0x05, (byte) 0x4a,
            // str r2, [ip, #12]
            (byte) 0xcc, (byte) 0xf8, (byte) 0x0c, (byte) 0x20, 
            // move r2, r9
            (byte) 0x4a, (byte) 0x46,                           
            // move r2, r9
            (byte) 0x4a, (byte) 0x46,                           
            ...
            // ldr pc, [pc, #4]
            (byte) 0xdf, (byte) 0xf8, (byte) 0x04, (byte) 0xf0, 
        ...
        writeInt((int) targetEntry, ByteOrder.LITTLE_ENDIAN, 
                 instructions, instructions.length - 12);

        writeInt((int) srcAddress, ByteOrder.LITTLE_ENDIAN, 
                 instructions, instructions.length - 8);

        writeInt((int) structAddress, ByteOrder.LITTLE_ENDIAN, 
                 instructions, instructions.length - 4);
